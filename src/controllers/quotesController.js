...
const updateQuote = async (req, res) => {
  ...
  try {
    ...
    const updateData = {};
    if (typeof customer_name !== 'undefined') updateData.customer_name = customer_name;
    if (typeof truck_id !== 'undefined') updateData.truck_id = truck_id;
    if (typeof origin !== 'undefined') updateData.origin = origin;
    if (typeof destination !== 'undefined') updateData.destination = destination;
    if (typeof distance !== 'undefined') updateData.distance = distance;
    if (typeof status !== 'undefined') updateData.status = status;
    if (Object.keys(updateData).length === 0) {
      return res.status(400).json({ error: 'Sin campos para actualizar.' });
    }
    const [updatedQuote] = await db('quotes').where({ id }).update(updateData).returning('*');
